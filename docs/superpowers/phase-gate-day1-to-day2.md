...
## Day-3 grilling decisions (SETTLED 2026-08-09)

- **Purpose = AUGMENTATION.** The synthetic data exists to improve a detector. Headline = **augmentation lift** (detector on `real+synth` vs `real-alone`, both tested on the untouched holdout), NOT pure TSTR. TSTR (synth-alone → real) demoted to a supporting diagnostic.
- **Headline experiment = SCARCITY CURVE.** Train real-only vs real+synth at increasing real amounts (1,2,4,8,13 fit campaigns); plot holdout AUC-PR of both vs amount of real. Story: synth substitutes for scarce real (lift largest at small real, converging as real grows).
- **Memorization/novelty is now CORE** (augmentation only lifts if synth adds coverage beyond the 13 fit campaigns). Metric: Jaccard of each synth campaign's `(foothold,target)` edge-set vs every fit campaign → report % near-duplicates + overall novel-edge rate. Add to V2.
- **Benign source:** one streaming pass over `auth.txt.gz`, **windowed to the split** — fit-window benign → train negatives, holdout-window benign → test negatives. Random draw ~1–2M rows/window. Note: the 70/30 split was on *malicious* red-team campaigns (both fit and holdout are malicious); benign is a separate axis never persisted at event level in Day-1.
- **Benign label noise:** cheap filter — drop any benign row whose `(src,dst)` exactly matches a known redteam edge; document residual noise (`redteam.txt` is detection-incomplete, so "benign" may hide undetected attacks). For the lift metric this noise hits both arms and partially cancels.
- **Base rate:** sweep 1:100 / 1:1000 / 1:10000 injected malicious:benign, report AUC-PR at each, lead with 1:10000 (most realistic tractable). True 7e-7 is intractable for a 51-positive holdout.
- **Hourly placement:** weight campaign start by benign `hourly_volume`; verify resulting footprint ≈ real red-team 6–17 business hours (they agree).
- **`hop_index` in labels:** repurpose as within-campaign emission order (fan-out has no hops).
- **Day-4 first-class additions:** the 3 SPEC baselines + a **structural-only-feature ablation** (drop the constant `auth_type` etc.; if detection survives, signal is real structure not the NTLM shortcut) + **multi-seed CIs** + an honest bounded-limitations section.
- **Day-2 gap disposition:** FIX gap #2 (enforce creds-per-campaign: sample `m` from the real distribution, cap the harvest pool). KEEP gap #3 (broad foothold pool — the novelty that drives augmentation lift; intentional, document). DEFER gap #5 (multi-origin campaigns) and gap #1 (cred→target proxy) to limitations. TVAE stretch skipped.
